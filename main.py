...
from app.database_config import engine, get_db, db_type
...
if not groq_api_key:
    print("WARNING: GROQ_API_KEY not found in environment variables. Summary generation will fail.")
...
def generate_summary(req: schemas.SummaryRequest, db: Session = Depends(get_db)):
    ...
    if summary_chain is None:
        raise HTTPException(
            status_code=500, 
            detail="LLM not initialized. Check server logs for details and verify your GROQ_API_KEY."
        )
    ...
    try:
        ...
        print(f"Generating summary for movie: {movie.title}")
        print(f"With actors: {actor_list}")
